module AccessesHelper
  ...
  def board_watchers_list(board)
    ...
    tag.div(class: "divider divider--fade") do
      tag.strong(watchers.any? ? I18n.t("helpers.accesses.watching_title") : I18n.t("helpers.accesses.no_watchers"), class: "txt-uppercase")
    end +
    tag.div(avatar_tags(displayed_watchers), class: "board-tools__watching") do
      tag.div(data: { controller: "dialog", action: "keydown.esc->dialog#close click@document->dialog#closeOnClickOutside" }) do
        tag.button("+#{overflow_count}", class: "overflow-count btn btn--circle borderless", data: { action: "dialog#open" }, aria: { label: I18n.t("helpers.accesses.show_more_watchers", count: overflow_count) }) +
  ...
  def involvement_button(board, access, show_watchers, icon_only)
    label_text = access.access_only? ? I18n.t("helpers.accesses.watch_this") : I18n.t("helpers.accesses.stop_watching")
    button_to(
